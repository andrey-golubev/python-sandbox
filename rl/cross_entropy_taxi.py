...
def update_policy(elite_states, elite_actions, n_states, n_actions):
    ...
    for (elite_state, elite_action) in zip(elite_states, elite_actions):
        new_policy[elite_state][elite_action] += 1
    for i, actions in enumerate(new_policy):
        s = np.sum(actions)
        if s == 0:
            new_policy[i] = np.ones(n_actions)/n_actions
        else:
            new_policy[i] = new_policy[i]/s
...
def rl_cross_entropy():
    ...
    policy = np.divide(np.ones((n_states, n_actions)), n_actions)
...
if __name__ == '__main__':
    ...
    policy = np.divide(np.ones((500, 6)), 6)
    check_policy_init(policy)
