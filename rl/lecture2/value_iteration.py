...
def get_action_value(mdp, state_values, state, action, gamma):
    ...
    # rewards = [mdp.get_reward(state, action, next_state) for next_state in next_states.keys()]
    # np.add(rewards, state_values)
    return np.sum([prob * (mdp.get_reward(state, action, next_state) \
           + gamma*state_values[next_state]) for next_state, prob in next_states.items()])
...
def get_new_state_value(mdp, state_values, state, gamma):
    ...
    if mdp.is_terminal(state):
        return 0
    return np.max([get_action_value(mdp, state_values, state, a, gamma) for a in mdp.get_possible_actions(state)])
