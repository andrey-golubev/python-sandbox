...
def define_network(state_dim, n_actions):
    ...
    network.add_module('fc1', nn.Linear(state_dim[0], 27))
    network.add_module('relu1', nn.ReLU())
    network.add_module('fc2', nn.Linear(27, 17))
    network.add_module('relu2', nn.ReLU())
    network.add_module('fc3', nn.Linear(17, 10))
    network.add_module('relu3', nn.ReLU())
    network.add_module('fc4', nn.Linear(10, n_actions))
    return network
...
def compute_td_loss(states, actions, rewards, next_states, is_done, gamma=0.99, check_shapes=False):
    ...
    next_state_values, _ = predicted_next_qvalues.max(dim=1)  # < YOUR CODE HERE >
...
if __name__ == '__main__':
    ...
    for i in range(max_epochs):
        session_rewards = [generate_session(epsilon=epsilon, train=True) for _ in range(n_sessions)]
        ...
        epsilon = epsilon_threshold if epsilon_threshold > epsilon else epsilon
    ...
    if record_video:
        ...
        sessions = [generate_session(epsilon=epsilon, train=False) for _ in range(n_sessions)]
    env.close()