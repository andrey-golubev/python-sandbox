...
class ExpectedValueSarsaAgent:
    ...
    def get_state_value(self, state):
        ...
        return max([self.get_q_value(state, a) for a in possible_actions])

    def get_avg_q_value(self, state):
        """
          Returns E(Q(state,action))
          where the expectation is over legal actions.
        """
        possible_actions = self.get_legal_actions(state)
        if len(possible_actions) == 0:
          return 0.0

        return np.mean([self.get_q_value(state, a) for a in possible_actions])
    ...
    def get_best_policy_action(self, state):
        ...
        best_action_index = np.argmax([self.get_q_value(state, a) for a in possible_actions])
    ...
    def update(self, state, action, next_state, reward):
        ...
        """
        hat_q = reward + self.gamma * self.get_avg_q_value(next_state)
        updated_q = self.alpha * hat_q + (1 - self.alpha) * self.get_q_value(state, action)
