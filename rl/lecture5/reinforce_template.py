...
class ReinforceAgent(nn.Module):
    def __init__(self, state_dim, n_actions):
        ...
        self.fc1 = nn.Linear(state_dim[0], 40)
        self.fc2 = nn.Linear(40, 20)
        self.fc3 = nn.Linear(20, n_actions)
...
def predict_proba(states):
    ...
    probs = F.softmax(agent(states), dim=1)
...
def train_on_session(optimizer, states, actions, rewards, gamma=0.99):
    ...
    J_hat = torch.mean(logprobas_for_actions * Variable(torch.FloatTensor(get_cumulative_rewards(rewards))))
